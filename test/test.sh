#!/usr/bin/env bash
# trustmebro test harness , offline (a throwaway fixture git repo). the one network test is guarded.
# exits nonzero on any failure.
set -uo pipefail
TMB="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")/.." && pwd)/trustmebro"
fails=0
ok()  { printf 'ok   - %s\n' "$1"; }
bad() { printf 'FAIL - %s\n' "$1"; fails=$((fails+1)); }
...
echo "# trustmebro test on $(uname -s 2>/dev/null)"

# 1. permalink, line range, git@ remote, pushed HEAD
got="$("$TMB" permalink "$fix" src.txt 2 4 2>/dev/null)"
...
got="$("$TMB" permalink "$fix" src.txt 3 2>/dev/null)"
...
out="$("$TMB" permalink "$fix" src.txt 99 2>/dev/null)"; rc=$?
...
out="$("$TMB" permalink "$fix" nope.txt 1 2>/dev/null)"; rc=$?
...
out="$("$TMB" permalink "$fix" src.txt 1 2>/dev/null)"; rc=$?
...
got="$("$TMB" permalink "$fix" src.txt 2 4 --ref origin/main 2>/dev/null)"
exp="https://github.com/acme/widget/blob/$sha/src.txt#L2-L4"
...
out="$("$TMB" permalink "$fix" src.txt 6 --ref origin/main 2>/dev/null)"; rc=$?
...
pf="$("$TMB" preflight "$fix" 2>/dev/null)"
...
if curl -fsS -o /dev/null --max-time 8 https://example.com 2>/dev/null; then
  "$TMB" verify https://example.com >/dev/null 2>&1 && ok "verify 200" || bad "verify should pass on example.com"
  "$TMB" verify https://example.com/definitely-not-a-real-page-9f3a2b >/dev/null 2>&1 && bad "verify should fail on 404" || ok "verify rejects 404"
  printf '%s\n' https://example.com | "$TMB" verify - >/dev/null 2>&1 && ok "batch verify passes when all 200" || bad "batch verify should pass on all-200"
  printf '%s\n%s\n' https://example.com https://example.com/definitely-not-a-real-page-9f3a2b | "$TMB" verify - >/dev/null 2>&1 && bad "batch verify should fail if any url fails" || ok "batch verify fails if any url fails"
else
...
"$TMB" prove "$pf" >/dev/null 2>&1 && ok "prove passes on url-only fix" || bad "prove url-only fix should pass"
# (b) add a new link around an existing word -> still markup-only -> passes
perl -i -pe 's/\bdoor\b/[door](https:\/\/example.com\/door)/' "$pf"
"$TMB" prove "$pf" >/dev/null 2>&1 && ok "prove passes when a link is added" || bad "prove add-link should pass"
# (c) a prose word change -> FAILS
perl -i -pe 's/\bcat\b/dog/' "$pf"
"$TMB" prove "$pf" >/dev/null 2>&1 && bad "prove should FAIL on prose change" || ok "prove fails on prose change"
# (d) a link-TEXT change is a prose change -> FAILS
git -C "$fix" checkout -q post.md
perl -i -pe 's/\[mat\]/[rug]/' "$pf"
"$TMB" prove "$pf" >/dev/null 2>&1 && bad "prove should FAIL on link-text change" || ok "prove fails on link-text change"
# (e) a CRLF flip (whole-file whitespace churn) -> FAILS
git -C "$fix" checkout -q post.md
perl -i -pe 's/\n/\r\n/' "$pf"
"$TMB" prove "$pf" >/dev/null 2>&1 && bad "prove should FAIL on CRLF churn" || ok "prove fails on line-ending churn"
git -C "$fix" checkout -q post.md
...
"$TMB" prove "$pf" >/dev/null 2>&1 && ok "prove passes on url-as-text fix (href+text)" || bad "prove should pass when only urls changed"
git -C "$fix" checkout -q post.md
...
perl -i -pe 's#https://old.example/x#https://new.example/y#' "$pf"
"$TMB" prove "$pf" >/dev/null 2>&1 && bad "prove should FAIL on a bare-url change (reader-visible)" || ok "prove fails on a bare-url text change"
git -C "$fix" checkout -q post.md
...
perl -i -pe 's#https://old.example/x#https://new.example/y#' "$pf"
"$TMB" prove "$pf" >/dev/null 2>&1 && bad "prove should FAIL on an autolink change" || ok "prove fails on an autolink change"
git -C "$fix" checkout -q post.md
...
"$TMB" links "$pb" | grep -qx 'https://example.com/p' && ok "links: paren-wrapped bare url extracted" || bad "links: paren-wrapped bare url missed"
rm -f "$pb"
...
"$TMB" lint "$lf" >/dev/null 2>&1 && bad "lint should FAIL on a url-text != href" || ok "lint flags markdown half-fix"
printf 'b <a href="https://new.example/y">https://old.example/x</a> here.\n' > "$lf"
"$TMB" lint "$lf" >/dev/null 2>&1 && bad "lint should FAIL on html half-fix" || ok "lint flags html half-fix"
printf 'c [label](https://ok.example/z) and [https://same.example/q](https://same.example/q).\n' > "$lf"
"$TMB" lint "$lf" >/dev/null 2>&1 && ok "lint clean when text!=url or text==href" || bad "lint should pass (descriptive text + matching url-text)"
rm -f "$lf"

# 14. flag: appends a durable line to .tmb-flags.md at the repo root
ff="$fix/flagme.md"; printf 'x\n' > "$ff"; git -C "$fix" add flagme.md; git -C "$fix" commit -qm flagme
"$TMB" flag "$ff" "dead pre-existing link foo.example" >/dev/null 2>&1
{ [ -f "$fix/.tmb-flags.md" ] && grep -q 'dead pre-existing link foo.example' "$fix/.tmb-flags.md"; } && ok "flag appends to .tmb-flags.md" || bad "flag should append to .tmb-flags.md"
rm -f "$ff" "$fix/.tmb-flags.md"
...
"$TMB" lint "$mm" >/dev/null 2>&1 && ok "lint: image alt-url is not a half-fix" || bad "lint: image false-positive not fixed"
printf '%s\n' "<a href='https://new.example/y'>https://old.example/x</a>" > "$mm"
"$TMB" lint "$mm" >/dev/null 2>&1 && bad "lint: single-quoted html half-fix missed" || ok "lint: single-quoted html half-fix caught"
rm -f "$mm"
# permalink: reject line 0 and a backwards range (lower-bound + order checks)
out="$("$TMB" permalink "$fix" src.txt 0 2>/dev/null)"; rc=$?
{ [ -z "$out" ] && [ "$rc" -ne 0 ]; } && ok "permalink rejects line 0" || bad "permalink should reject line 0"
out="$("$TMB" permalink "$fix" src.txt 4 2 2>/dev/null)"; rc=$?
...
"$TMB" permalink "$fix" nonl.txt 3 --ref origin/main >/dev/null 2>&1 && ok "permalink accepts last line w/o trailing newline" || bad "permalink should accept line 3 of a no-trailing-newline file"
...
"$TMB" sweep "$fix" "$swbase" >/dev/null 2>&1 && bad "sweep should FAIL (swb half-fix)" || ok "sweep fails when a file has a half-fix"
swout="$("$TMB" sweep "$fix" "$swbase" 2>/dev/null)"
...
jout="$("$TMB" links "$j" --json 2>/dev/null)"
...
"$TMB" lint "$j" --fix >/dev/null 2>&1
"$TMB" lint "$j" >/dev/null 2>&1 && ok "lint --fix repaired the half-fix" || bad "lint --fix should leave it clean"
...
printf '%s\n' "see [http://old.example/x](https://new.example/y) ok" > "$j"
ljson="$("$TMB" lint "$j" --json 2>/dev/null)"
...
"$TMB" check "$fix/c.md" HEAD >/dev/null 2>&1 && ok "check PASS on a clean post" || bad "check should PASS on a clean post"
perl -i -pe 's/prose/PROSE-EDIT/' "$fix/c.md"
"$TMB" check "$fix/c.md" HEAD >/dev/null 2>&1 && bad "check should report ISSUES on a prose change" || ok "check fails on a prose change"
...
"$TMB" links "$rs" | grep -qx 'https://example.com/ref' && ok "links: reference-style def extracted" || bad "links: ref-style def missed"
rm -f "$rs"
"$TMB" version | grep -q '^trustmebro [0-9]' && ok "version prints" || bad "version should print"
"$TMB" version --json | grep -q '"version"' && ok "version --json" || bad "version --json"
djson="$("$TMB" lint /no/such/file --json 2>/dev/null)"
...
cout="$("$TMB" check "$fix/nf.md" HEAD 2>/dev/null)"
...
if curl -fsS -o /dev/null --max-time 8 https://example.com 2>/dev/null; then
  ...
  printf '%s\n' "$long" | "$TMB" verify - >/dev/null 2>&1 && bad "batch verify should catch a >255-byte dead url" || ok "batch verify catches a long dead url (no xargs -I drop)"
...
"$TMB" flag "$fix/flk.md" "known dead: https://flagged-zzz.invalid" >/dev/null 2>&1
cflag="$("$TMB" check "$fix/flk.md" HEAD 2>/dev/null)"
...
"$TMB" links "$mlx" | grep -qx 'https://ml.example/x' && ok "links: multi-line link text (slurp mode)" || bad "links: multi-line link text missed"
...
"$TMB" links "$mlx" | grep -q 'commented.example' && bad "links: html-comment link should be masked" || ok "links: html comment masked"
rm -f "$mlx"
...
"$TMB" flag "$fix/pre.md" "known dead: https://dead-zzz.invalid/page-two" >/dev/null 2>&1
pout="$("$TMB" check "$fix/pre.md" HEAD 2>/dev/null)"
...
if curl -fsS -o /dev/null --max-time 8 https://example.com 2>/dev/null; then
  "$TMB" insert "$ins" "raft phrase" https://example.com >/dev/null 2>&1
  grep -qF '[raft phrase](https://example.com)' "$ins" && ok "insert wraps a unique match" || bad "insert should wrap a unique phrase"
  out2="$("$TMB" insert "$ins" "token bucket" https://example.com 2>&1)"; rc2=$?
...
out="$("$TMB" insert "$cz" "reqlib" https://example.com 2>&1)"; rc=$?
...
"$TMB" prove "$cz" HEAD >/dev/null 2>&1 && bad "prove should FAIL on an in-code link injection" || ok "prove catches a link injected into code"
# double-insert leaves no nested [[..](..)](..)
printf 'we use widgetlib in prod.\n' > "$cz"; git -C "$fix" add cz.md; git -C "$fix" commit -qm cz3
if curl -fsS -o /dev/null --max-time 8 https://example.com 2>/dev/null; then
  "$TMB" insert "$cz" "widgetlib" https://example.com >/dev/null 2>&1
  "$TMB" insert "$cz" "widgetlib" https://example.com >/dev/null 2>&1
...
# TMB_JOBS validation: a bad value must die LOUD at dispatch, not silently disable verification (offline,
# the dispatch guard fires before any network). guards the fail-open where a typo flips check ISSUES->PASS.
out="$(printf 'https://x\n' | TMB_JOBS=garbage "$TMB" verify - 2>/dev/null)"; rc=$?
{ [ -z "$out" ] && [ "$rc" -ne 0 ]; } && ok "TMB_JOBS=garbage dies (no silent fail-open)" || bad "TMB_JOBS=garbage should die (out='$out' rc=$rc)"
out="$(printf 'https://x\n' | TMB_JOBS=-5 "$TMB" verify - 2>/dev/null)"; rc=$?
{ [ -z "$out" ] && [ "$rc" -ne 0 ]; } && ok "TMB_JOBS=-5 dies" || bad "TMB_JOBS=-5 should die (rc=$rc)"
...
out="$(TMB_JSON=1 "$TMB" links "$jt" 2>/dev/null)"
...
"$TMB" lint "$lw" >/dev/null 2>&1 && bad "lint missed a leading-whitespace half-fix (fail-open)" || ok "lint catches a leading-whitespace url-as-text half-fix"
rm -f "$lw"

# a directory path gives a clean 'no file' error, not a raw node EISDIR leak (the lost [ -f ] check)
out="$("$TMB" links "$fix" 2>&1)"; rc=$?
...
"$TMB" prove "$nlf" HEAD >/dev/null 2>&1 && ok "prove ignores a trailing-newline-only delta" || bad "prove false-failed on an EOF newline"
...
"$TMB" insert "$al" Paxos https://example.com >/dev/null 2>&1 && bad "insert wrapped inside an autolink" || ok "insert refuses a phrase only inside an autolink"
...
"$TMB" insert "$im" "temperature rise" https://example.com >/dev/null 2>&1 && bad "insert wrapped inside image alt text" || ok "insert refuses a phrase only inside image alt text"
...
"$TMB" insert "$li" landingpage https://example.org >/dev/null 2>&1 && bad "insert wrapped a linked-image outer href" || ok "insert refuses inside a linked-image outer href"
...
"$TMB" insert "$ht" brandlogo https://example.org >/dev/null 2>&1 && bad "insert wrapped inside an <img> tag" || ok "insert refuses inside an html <img> tag"
...
"$TMB" insert "$hp" "example" https://example.org >/dev/null 2>&1 && bad "insert wrapped a phrase only inside html <pre><code>" || ok "insert refuses a phrase only inside html <pre>/<code>"
"$TMB" insert "$hp" "raft" https://raft.github.io >/dev/null 2>&1
...
# TMB_JOBS is scoped to verify/check/sweep: a stray value must NOT kill non-parallel commands...
out="$(TMB_JOBS=auto "$TMB" prove "$fix/src.txt" HEAD 2>&1)"
printf '%s' "$out" | grep -q 'TMB_JOBS' && bad "TMB_JOBS falsely gates prove" || ok "TMB_JOBS does not gate non-parallel commands"
# ...but it DOES still gate verify.
out="$(printf 'https://x\n' | TMB_JOBS=auto "$TMB" verify - 2>&1)"; rc=$?
{ [ "$rc" -ne 0 ] && printf '%s' "$out" | grep -q 'TMB_JOBS'; } && ok "TMB_JOBS still gates verify" || bad "TMB_JOBS should gate verify (out='$out' rc=$rc)"

# `--` ends option parsing: a reason/phrase that IS a flag token must be taken literally, not swallowed.
rm -f "$fix/.tmb-flags.md"
"$TMB" flag "$fix/src.txt" -- --json --fix >/dev/null 2>&1
grep -q -- '--json --fix' "$fix/.tmb-flags.md" 2>/dev/null && ok "-- escapes flag tokens (literal reason recorded)" || bad "-- should record '--json --fix' as a literal reason"
